...
class DB:
    ...
    async def add_phone_number(self, user_id: int, phone_number: str,
                               tariff_price: Optional[float] = None,
                               tariff_label: Optional[str] = None) -> dict:
        try:
            ...
            if phone_history is None:
                add = await self.db.fetchval(
                    "INSERT INTO queue (user_id, phone_number, tariff_price, tariff_label) "
                    "VALUES ($1, $2, $3, $4) RETURNING _id",
                    user_id, phone_number, tariff_price, tariff_label
                )
                if not add:
                ...
                return {"status": True, "msg": position, "number_id": add}
            ...
            add = await self.db.fetchval(
                "INSERT INTO queue (user_id, phone_number, tariff_price, tariff_label) "
                "VALUES ($1, $2, $3, $4) RETURNING _id", user_id, phone_number, tariff_price, tariff_label
            )
            if not add:
                return {"status": False, "msg": "не удалось добавить номер в базу данных"}
            position = await self.db.fetchval(
    ...
    async def mark_queue_paid(self, _id: int, amount: float):
        await self.db.execute(
            "UPDATE queue SET status = 'paid', payout_amount = $1, paid_at = now() WHERE _id = $2",
            amount, _id
        )
